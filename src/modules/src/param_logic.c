...
{
  intptr_t valueRelative;
  intptr_t address;
  void *ptrDefaultValue;

  address = (intptr_t)(params[index].address);

  // Is variable in data section?
  if (address >= (intptr_t)&_sdata &&
      address <= (intptr_t)&_edata)
  {
    valueRelative =  address - (intptr_t)&_sdata;
    ptrDefaultValue = (void *)((intptr_t)&_sidata + valueRelative);
  }
  // Is variable in flash section?
  else if (address >= (intptr_t)&_stext &&
           address <= (intptr_t)&_etext)
  {
