  ...
  while(1)
  {
    ...
    if (fds[0].revents & POLLIN){
      ...
      if (recvlen > 0){
        ...
        xQueueSend(crtpPacketDelivery, &p, 0);  // silently drop if queue full
      } else {
        DEBUG_PRINT("error : %s \n" , strerror(errno));
      }
    } 
...
void socketlinkInit()
{
  ...
  // Set receive timeout so we don't block forever in handshake
  struct timeval tv;
  tv.tv_sec = 0;
  ...
  // Increase UDP buffers to 2MB to handle 10-drone swarm TOC initialization bursts
