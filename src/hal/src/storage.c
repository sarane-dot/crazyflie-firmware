...
void storagePrintStats()
{
  ...
  DEBUG_PRINT("Used storage: %zu item stored, %zu Bytes/%zu Bytes (%d%%)\n", stats.totalItems, stats.itemSize, stats.totalSize, (int)((stats.itemSize*100)/stats.totalSize));
  DEBUG_PRINT("Fragmentation: %d%%\n", (int)stats.fragmentation);
  DEBUG_PRINT("Efficiency: Data: %zu Bytes (%d%%), Keys: %zu Bytes (%d%%), Metadata: %zu Bytes (%d%%)\n",
    stats.dataSize, (int)((stats.dataSize*100)/stats.totalSize),
    stats.keySize, (int)((stats.keySize*100)/stats.totalSize),
    stats.metadataSize, (int)((stats.metadataSize*100)/stats.totalSize));
}
